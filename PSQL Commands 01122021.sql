...
COPY ictrp."ictrp_weekly_data" FROM 'C:\Users\vamsi\Desktop\ICTRP\weekly data extracted\ICTRPWeek22November2021.csv'
DELIMITER ',' CSV QUOTE AS '"' HEADER; -- 54584 records

COPY ictrp."ictrp_weekly_data" FROM 'C:\Users\vamsi\Desktop\ICTRP\weekly data extracted\ICTRPWeek15November2021.csv'
DELIMITER ',' CSV QUOTE AS '"' HEADER; -- 17271 records
...
select * from ictrp.ictrp_weekly_data; -- 119857 records
...
select * from ictrp.ictrp_full_weekly_data; --1198557 records
...
select * from ictrp.ictrp_full_weekly_data; -- 1846375 records
...
-- ictrp.ictrp_full_weekly_data : count - 1867272 records
-- ictrp.ictrp_weekly_data : count - 1219454 records
...
select count(distinct (trialid))  from ictrp.ictrp_full_weekly_data; -- 735344 records

select trialid,count(*)  from ictrp.ictrp_full_weekly_data
group by trialid  having count(*) > 1 order by 2 desc; -- 45 is highest 

select count(*) from ictrp.ictrp_full_weekly_data 
where  trialid  in ('NTR1315','NTR2804','NTR986','NTR5983','NTR4826','NTR3117'); -- 270

select count(*) from ictrp.ictrp_full_export
where  trialid  in ('NTR1315','NTR2804','NTR986','NTR5983','NTR4826','NTR3117'); -- 6

select distinct "(No column name)" from ictrp.ictrp_full_weekly_data ; -- 499 records
...
group by trialid; -- ACTRN12605000001695 (418101,2) 735344 records (these count of unique records)

-- command to delete the duplicates in 1131928 records
...
and i.trialid = t.trialid; -- 1131928 records are deleted 

select trialid, count(*) from ictrp.ictrp_full_weekly_data
group by trialid; --735344 records are there in a table at present
...
SELECT * FROM ictrp.ictrp_full_weekly_data; -- query executed with 73534 records 

select trialid, count(*) from ictrp_rpt.ictrp_full_data
group by trialid; -- 735344 records are there in new table 
 ...
 phase = trim(both '" "' from phase) , 
  date_registration = trim(both '" "' from date_registration) , 
   date_enrollement = trim(both '" "' from date_enrollement) , 
	...
	,target_size = trim(both '" "' from target_size); -- 735344 records updated
...
update ictrp_rpt.ictrp_full_data set conditions  = trim(leading ';' from conditions) ; -- 735344 records
update ictrp_rpt.ictrp_full_data set conditions  = trim(both '<br>' from conditions) ; -- 735344 records
...
where  conditions  ilike 'Stroke%'; -- 4370 updated

update ictrp_rpt.ictrp_full_data set conditions_std  = 'Healthy Volunteers' 
where  conditions  = 'Healthy'; -- 5811 updated
...
ilike  'NULL%' or length(conditions) = 0; -- 11051 updated

update ictrp_rpt.ictrp_full_data set conditions_std  = 'Non-Small Cell Lung Cancer' 
where  conditions  ilike 'Non-small Cell Lung Cance%'; -- 2690 updated

update ictrp_rpt.ictrp_full_data set conditions_std  = 'Pancreatic Cancer' 
where  conditions  ilike 'Pancreatic Cance%'; -- 1465 updated

update ictrp_rpt.ictrp_full_data set conditions_std  = 'Cancer' 
where  conditions  ilike 'Cance'; -- 1256 updated

update ictrp_rpt.ictrp_full_data set conditions_std  = 'Healthy Volunteers' 
where  conditions  ilike 'Healthy adult%' or conditions  ilike  'Healthy Volunteer%'; -- 5255 updated
--- CONDITIONS END -----
...
update ictrp_rpt.ictrp_full_data d
	...
	where  d.trialid  like '%'||m.source_register_cd||'%' and source_register_cd_drvd is null; --719683 updated

	update ictrp_rpt.ictrp_full_data d
	   set source_register_cd_drvd = 'NTR' 
     where source_register_cd_drvd is null and trialid like 'NL%'; -- 2335 updated
...
update ictrp01122021.ictrp_rpt.ictrp_full_data d
...
and length (target_size) > 0 and target_size  is not null and target_size  <> 'null'; -- 674635 updated
